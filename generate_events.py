#!/usr/bin/env python3
"""
Sports Events JSON Generator v5
- Properly parses embedhd.org nested API (days → items)
- Uses ts_et (Unix timestamp) for event times
- Proper league name formatting (FIFA World Cup, not "fifa world cup")
- Cricket teams from cricket API, streams from M3U playlists only
...
# ─── League Name Mapping ────────────────────────────────────────────────────
# Maps lowercase/raw league names to their proper display names.
# Checked via substring matching and exact matching.
...
LEAGUE_NAME_MAP = {
    ...
    "women's premier league": "Women's Premier League",
    ...
    "royal london one-day cup": "Royal London One-Day Cup",
    "asia cup": "Asia Cup",
    ...
    "syed mushtaq ali": "Syed Mushtaq Ali Trophy",
    ...
    "pak vs aus": "Pakistan vs Australia",
    ...
    "boxing": "Boxing",
    "wba boxing": "WBA Boxing",
    "wbc boxing": "WBC Boxing",
    "wwe": "WWE",
    "wwe raw": "WWE Raw",
    ...
    "ittf": "ITTF Table Tennis",
    "badminton": "Badminton",
    "bwf": "BWF Badminton",
    "volleyball": "Volleyball",
    "fivb": "FIVB Volleyball",
    "handball": "Handball",
    "ehf": "EHF Handball",
    "swimming": "Swimming",
    "athletics": "Athletics",
...
def format_league_name(raw_league):
    """
    Convert a raw league name to its proper display name.
    Uses exact match first, then substring match, then smart title case.
    """
    if not raw_league:
        return ""
    ...
    # 2. Try with common prefixes/suffixes stripped
    cleaned = raw_lower
    ...
    # 3. Substring match — find the longest matching key
    best_match = None
    best_len = 0
    for key, display_name in LEAGUE_NAME_MAP.items():
    ...
    # 4. Smart title case with acronym preservation
    return smart_title_case(raw)


def smart_title_case(text):
    """
    Title case with special handling for acronyms and sports terms.
    """
    if not text:
        return ""
    ...
    ACRONYMS = {
        ...
        'fifa', 'uefa', 'icc', 'afc', 'caf',
        'f1', 'wrc', 'wec', 'dtm',
        'pga', 'lpga', 'wta', 'atp', 'itf',
        'nrl', 'urc',
        'pdc', 'bwf', 'fivb', 'ehf', 'ittf', 'fiba',
        'nascar', 'indycar',
        'usa', 'uk', 'uae', 'khl',
        'sa20', 'ilt20', 't20', 'odi',
        'dfb', 'dfl', 'efl',
    ...
    # Known lowercase words (unless first word)
    LOWERCASE_WORDS = {
    ...
    for i, word in enumerate(words):
        ...
        if word_clean in ACRONYMS:
            # Keep as uppercase acronym
            result.append(word.upper())
        elif i > 0 and word_lower in LOWERCASE_WORDS:
            # Lowercase connecting words (but not first word)
            ...
            for p in parts:
                p_lower = p.lower()
                p_clean = re.sub(r'[^a-z0-9]', '', p_lower)
                if p_clean in ACRONYMS:
                    titled_parts.append(p.upper())
                else:
...
SPORT_KEYWORDS = [
    ...
    ("f1 qualifying", "Formula 1"), ("f1 practice", "Formula 1"),
    ("f1 sprint", "Formula 1"), ("grand prix", "Formula 1"),
    ("motogp", "MotoGP"), ("moto gp", "MotoGP"),
    ("moto2", "MotoGP"), ("moto3", "MotoGP"),
    ("nascar", "NASCAR"), ("indycar", "IndyCar"),
    ("formula e", "Formula E"), ("formula-e", "Formula E"),
    ("wrc", "Rally"), ("rally", "Rally"),
    ...
    ("psl cricket", "Cricket"), ("cpl", "Cricket"),
    ("the hundred", "Cricket"), ("sa20", "Cricket"),
    ("ilt20", "Cricket"), ("bpl", "Cricket"),
    ("mlc t20", "Cricket"), ("wpl", "Cricket"),
    ("county championship", "Cricket"), ("asia cup", "Cricket"),
    ("champions trophy", "Cricket"), ("ashes", "Cricket"),
...
F1_INDICATORS = [
    ...
    "alpine", "williams", "haas", "alphatauri", "sauber", "kick sauber",
    "verstappen", "hamilton", "leclerc", "norris", "sainz", "perez",
    "alonso", "stroll", "piastri", "russell", "gasly", "ocon",
...
def detect_sport_type_smart(text_parts):
    ...
    f1_score = sum(1 for ind in F1_INDICATORS if ind in combined)
    if f1_score >= 1:
        has_football = any(
        ...
        if not has_football or f1_score >= 2:
            return "Formula 1"
...
TEAM_NAME_ALIASES = {
    ...
    "south-korea": ["korea republic", "korea", "south korea", "s. korea", "republic of korea"],
    "turkiye": ["turkey", "türkiye"], "curacao": ["curaçao", "curacao"],
    ...
    "costa-rica": ["costa rica"], "hong-kong": ["hong kong"],
    "england": ["england", "eng"], "australia": ["australia", "aus"],
    "india": ["india", "ind"], "pakistan": ["pakistan", "pak"],
    "bangladesh": ["bangladesh", "ban", "bd"],
    "afghanistan": ["afghanistan", "afg"],
    "zimbabwe": ["zimbabwe", "zim"], "ireland": ["ireland", "ire"],
...
def safe_request(url, timeout=30, retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            print(f"  [WARN] Attempt {attempt+1}/{retries} failed: {url[:80]}... : {e}")
...
def get_known_slug(name):
    ...
    for slug, aliases in TEAM_NAME_ALIASES.items():
        if norm == slug.replace('-', ' '): return slug
        for alias in aliases:
            if norm == alias.lower(): return slug
    return None


def get_team_logo_smart(team_name):
    known = get_known_slug(team_name)
    if known: return f"{CDN_BASE}/team/{known}/logo.webp"
    s = team_slug(team_name)
    if not s: return f"{CDN_BASE}/team/unknown/logo.webp"
...
def parse_m3u_playlists(playlist_urls):
    ...
    print(f"  Total playlist entries: {len(all_entries)}")
    return all_entries
...
def parse_m3u_content(lines):
    ...
    for line in lines:
        ...
        if line.startswith('#EXTINF:'):
            ...
            if m: current.tvg_name = m.group(1).strip()
            m = re.search(r'tvg-logo="([^"]*)"', line)
            if m: current.tvg_logo = m.group(1).strip()
            m = re.search(r'group-title="([^"]*)"', line)
            if m: current.group = m.group(1).strip()
            ...
            if '=' in opt:
                key, val = opt.split('=', 1)
                key = key.strip().lower()
                if key in ('http-referrer', 'http-referer'): pending['referer'] = val.strip()
                elif key == 'http-origin': pending['origin'] = val.strip()
                elif key == 'http-user-agent': pending['user_agent'] = val.strip()
            ...
            if '=' in prop:
                ...
                key = key.strip().lower()
                if 'license_type' in key: pending['drm_scheme'] = val.strip()
                elif 'license_key' in key: pending['drm_license'] = val.strip()
            ...
            try:
                ...
                for hk, hv in hdrs.items():
                    hkl = hk.lower()
                    if 'referer' in hkl: pending['referer'] = hv
                    elif 'origin' in hkl: pending['origin'] = hv
                    elif 'user-agent' in hkl: pending['user_agent'] = hv
                    else: pending[hk] = hv
            except Exception: pass
                ...
                if '|' in line:
                    ...
                    for hdr in parts[1].strip().split('&'):
                        if '=' in hdr:
                            hk, hv = hdr.split('=', 1)
                            hkl = hk.strip().lower()
                            if 'referer' in hkl: current.referer = hv.strip()
                            elif 'origin' in hkl: current.origin = hv.strip()
                            elif 'user-agent' in hkl: current.user_agent = hv.strip()
                            elif 'drmscheme' in hkl: current.drm_scheme = hv.strip()
                            elif 'drmlicense' in hkl: current.drm_license = hv.strip()
                            else: current.extra_headers[hk.strip()] = hv.strip()
                else:
                    current.url = line.strip()

                if 'referer' in pending and not current.referer: current.referer = pending['referer']
                if 'origin' in pending and not current.origin: current.origin = pending['origin']
                if 'user_agent' in pending and not current.user_agent: current.user_agent = pending['user_agent']
                if 'drm_scheme' in pending and not current.drm_scheme: current.drm_scheme = pending['drm_scheme']
                if 'drm_license' in pending and not current.drm_license: current.drm_license = pending['drm_license']
...
def build_stream_url(entry):
    ...
    if entry.drm_scheme:
        ...
        if 'clearkey' in s: parts.append("drmScheme=clearkey")
        elif 'widevine' in s: parts.append("drmScheme=widevine")
        elif 'playready' in s: parts.append("drmScheme=playready")
        else: parts.append(f"drmScheme={entry.drm_scheme}")
    if entry.drm_license: parts.append(f"drmLicense={entry.drm_license.strip()}")
    if entry.referer: parts.append(f"Referer={entry.referer}")
    if entry.origin: parts.append(f"Origin={entry.origin}")
    if entry.user_agent: parts.append(f"User-Agent={entry.user_agent}")
    elif entry.referer or entry.origin: parts.append(f"User-Agent={DEFAULT_USER_AGENT}")
...
def _name_in_text(original, norm, text):
    if not norm or len(norm) < 3: return False
    if norm in text: return True
    slug = get_known_slug(original)
    if slug:
        if slug.replace('-', ' ') in text: return True
        for alias in TEAM_NAME_ALIASES.get(slug, []):
            if alias.lower() in text: return True
    return False
...
def match_entry_to_event(entry, home, away, match_name, league=""):
    ...
    if hf and af: return True
    for sep in [' vs ', ' v ', ' - ', ' vs. ']:
        for a, b in [(hn, an), (an, hn)]:
            if a and b and f"{a}{sep}{b}" in text: return True
    if hn and an and len(hn) > 3 and len(an) > 3 and ' ' not in hn and ' ' not in an:
        if hn in text and an in text: return True
    mn = normalize_team_name(match_name)
    if mn and len(mn) > 8 and mn in text: return True
    return False
...
def find_streams_for_event(event, m3u_entries):
    ...
    for entry in m3u_entries:
        ...
        if match_entry_to_event(entry, event.get('homeTeamName', ''),
...
def build_event_streams(event, m3u_entries):
    ...
    seen = set()
    num = 1
    for api_s in event.get('api_streams', []):
        base = extract_base_url(api_s['url'])
        if base not in seen:
            seen.add(base)
            streams.append({"name": f"Server {num}", "url": api_s['url'], "isPrimary": True})
            num += 1
    ...
    for url in find_streams_for_event(event, m3u_entries):
        ...
        if base in seen or base == default_base: continue
        seen.add(base)
        streams.append({"name": f"Server {num}", "url": url, "isPrimary": len(streams) == 0})
        num += 1
    if not streams:
        streams.append({"name": DEFAULT_STREAM_NAME, "url": DEFAULT_STREAM_URL, "isPrimary": True})
    elif not any(s['isPrimary'] for s in streams):
        streams[0]['isPrimary'] = True
    return streams
...
def parse_embedhd_event(item):
    ...
    if not home_team and isinstance(teams_obj, dict):
        ...
        if isinstance(h, dict): home_team = h.get('name', '')
    if not away_team and isinstance(teams_obj, dict):
        a = teams_obj.get('away', {})
        if isinstance(a, dict): away_team = a.get('name', '')
    ...
    if not home_team and not away_team and match_name:
        for sep in [' - ', ' vs ', ' v ']:
            if sep in match_name:
                ...
                break
        if not home_team: home_team = match_name
    ...
    if isinstance(teams_obj, dict):
        if not home_logo:
            h = teams_obj.get('home', {})
            if isinstance(h, dict): home_logo = h.get('logo') or ""
        if not away_logo:
            a = teams_obj.get('away', {})
            if isinstance(a, dict): away_logo = a.get('logo') or ""
    if not home_logo: home_logo = get_team_logo_smart(home_team)
    if not away_logo: away_logo = get_team_logo_smart(away_team)

    # League — format properly
    ...
    if not start_time:
        d = item.get('date_et', '')
        if d: start_time = format_start_time(d)
    ...
    if isinstance(is_hot, str): is_hot = is_hot.lower() in ('true', '1', 'yes')
    ...
    for s in (item.get('streams') or []):
        if isinstance(s, dict):
            ...
            if link:
                hd = s.get('hd', '')
                api_streams.append({'name': f"HD {hd}" if hd else "Stream", 'url': link})
...
def fetch_cricket_events():
    ...
    if isinstance(data, dict) and 'matches' in data:
        ...
        for k in ['events', 'data', 'results']:
            if k in data and isinstance(data[k], list): raw = data[k]; break
        if raw is None: raw = [data]
    else: return []
...
def parse_cricket_event(item):
    match_name = (item.get('match name') or item.get('match_name') or
                  item.get('matchName') or item.get('title') or
                  item.get('name') or "")
    home_team = (item.get('Team 1 Name') or item.get('homeTeamName') or
                 item.get('home_team') or item.get('team1') or "")
    away_team = (item.get('Team 2 Name') or item.get('awayTeamName') or
    ...
    # League — format properly
    raw_league = (item.get('Tour/Group name') or item.get('league') or
    ...
    start_time_raw = (item.get('Start time') or item.get('start_time') or
...
def main():
    print("=" * 60)
    print("Sports Events JSON Generator v5")
    print("=" * 60)
    ...
    print(f"\nTotal streams: {total}")
    print(f"Events with real streams: {real}/{len(output_events)}")

    print(f"\nSample output (league names + times):")
    print("-" * 110)
    for e in output_events[:10]:
        print(f"  [{e['sportType']:12s}] {e['matchName'][:30]:30s} | {e['league'][:25]:25s} | {e['startTime']:22s}")
    print("-" * 110)
    print(f"{'=' * 60}")
